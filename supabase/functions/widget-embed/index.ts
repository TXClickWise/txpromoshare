...
serve(async (req) => {
  ...
  if (!widgetId) {
    return new Response("widget_id required", {
      status: 400,
      headers: corsHeaders,
    });
  }
  ...
  try {
    ...
    let query = supabase
      .from("events")
      .select(
        "id, title, subtitle, slug, start_date, start_time, end_date, end_time, short_description, status, cta_link, cta_button_text, featured_image_id",
      )
    ...
    const imageIds = (events || []).map((e: any) => e.featured_image_id).filter(
      Boolean,
    );
    ...
    const eventsWithImages = (events || []).map((e: any) => ({
      ...e,
      featured_image_url: e.featured_image_id
        ? imageMap[e.featured_image_id] || null
        : null,
    }));
...
function generateEmbedScript(payload: any): string {
  ...
  const eventCards = (events as any[]).map((e: any) => {
    ...
    if (e.cta_link) {
      ...
      ctaHtml = '<a href="' + escapeHtml(e.cta_link) +
        '" target="_blank" rel="noopener" style="display:inline-block;margin-top:8px;padding:6px 16px;background:' +
        pc +
    ...
    const subtitleHtml = e.subtitle
      ? '<div style="font-size:13px;color:#6b7280;margin-top:2px;">' +
        escapeHtml(e.subtitle) + "</div>"
      : "";
    const descHtml = e.short_description
      ? '<div style="font-size:13px;color:#374151;margin-top:6px;">' +
        escapeHtml(e.short_description) + "</div>"
      : "";
    ...
    const imageHtml = e.featured_image_url
      ? '<div style="display:flex;align-items:center;justify-content:flex-end;flex:0 0 auto;max-height:100px;max-width:220px;margin-left:auto;">' +
        '<img src="' + escapeHtml(e.featured_image_url) + '" alt="' +
        escapeHtml(e.title) +
        '" loading="lazy" style="display:block;width:auto;height:auto;max-height:100px;max-width:220px;object-fit:contain;border-radius:8px;flex-shrink:0;" />' +
        "</div>"
      : "";

    // Share buttons
    const eventUrl = e.cta_link ||
      ("https://txeventshare.nl/event/" + escapeHtml(e.slug));
    const shareText = encodeURIComponent(
      e.title + " — " + dateStr + " " + time,
    );
    ...
    const shareHtml = '<div style="display:flex;gap:6px;margin-top:10px;">' +
      ...
      '" target="_blank" rel="noopener" title="Deel via WhatsApp" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;background:#25D366;color:#fff;text-decoration:none;font-size:16px;">&#9993;</a>' +
      '<a href="https://www.facebook.com/sharer/sharer.php?u=' + shareUrl +
      '" target="_blank" rel="noopener" title="Deel op Facebook" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;background:#1877F2;color:#fff;text-decoration:none;font-size:14px;font-weight:700;">f</a>' +
      '<a href="https://twitter.com/intent/tweet?text=' + shareText + "&url=" +
      shareUrl +
      '" target="_blank" rel="noopener" title="Deel op X" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;background:#000;color:#fff;text-decoration:none;font-size:14px;font-weight:700;">𝕏</a>' +
      '<a href="mailto:?subject=' + shareText + "&body=" + shareText + "%20" +
      shareUrl +
  ...
  const noEventsHtml =
    '<p style="color:#6b7280;font-size:14px;">Geen aankomende evenementen.</p>';
  const innerHtml = events.length === 0 ? noEventsHtml : eventCards;

  const html =
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;max-width:600px;">' +
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;">' +
    '<div style="width:4px;height:24px;background:' + pc +
  ...
  return "(function(){" +
    ...
    "if(!c){var s=document.querySelectorAll('script[data-widget-id=\"" +
    widget.id + "\"]');" +
    'if(s.length>0){c=document.createElement("div");c.id="txeventshare-widget-' +
    widget.id + '";' +

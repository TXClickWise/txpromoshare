      ...
      {dragOver && (
        <div className="fixed inset-0 z-50 bg-primary/10 border-4 border-dashed border-primary rounded-xl flex items-center justify-center pointer-events-none">
          <div className="bg-background rounded-xl p-8 shadow-lg text-center">
            ...
            <p className="text-lg font-display font-bold text-foreground">Sleep bestanden hier</p>
            <p className="text-sm text-muted-foreground">om te uploaden naar je mediabibliotheek</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
        ...
        <Select value={sortMode} onValueChange={(v) => setSortMode(v as SortMode)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="size">Grootste eerst</SelectItem>
          </SelectContent>
        </Select>
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={toggleSelectAll} className="text-xs">
              {selectedIds.size === filtered.length ? "Deselecteer" : "Selecteer alles"}
            </Button>
      ...
      {isLoading ? (
        ...
        search || sourceFilter !== "all" ? (
          ...
          <EmptyState
            ...
            description="Upload afbeeldingen, sleep ze hierheen, of zoek in gratis stockfoto's."
            ...
            <div key={item.id} className="flex items-center gap-3 p-3 hover:bg-secondary/50 transition-colors group cursor-pointer"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground flex items-center gap-2 flex-wrap">
                  ...
                  {usageMap[item.id] && (
                    <Badge variant="secondary" className="text-[9px] h-4">{usageMap[item.id]}× in events</Badge>
                  )}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                {previewItem.alt_text && (
                  <div className="col-span-2">
                    <p className="text-muted-foreground">Alt-tekst</p>
                    <p className="font-medium text-foreground">{previewItem.alt_text}</p>
                  </div>
                )}
                {usageMap[previewItem.id] && (
                  <div>
                    <p className="text-muted-foreground">Gebruik</p>
                    <p className="font-medium text-foreground">{usageMap[previewItem.id]}× in events</p>
                  </div>
                )}
              </div>
              <div className="flex gap-2">
